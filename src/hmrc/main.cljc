(ns hmrc.main
  "Kotodama WASM entrypoint for the Hmrc clean-room actor (L4) — Clojure port.
  ...
  facts (`hmrc.<Entity>/<field>`); `*store*` is the in-memory materialization
...
(defn emit-facts
  "EAVT facts for one record: {\"hmrc.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "hmrc-compat" :tier tier :entities entities} 200])
